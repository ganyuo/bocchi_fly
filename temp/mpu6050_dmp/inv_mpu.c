...
/**
 * @brief 使用i2c写mpu6050的寄存器
 * 
 * @param slave_addr mpu6050的从机地址
 * @param reg_addr mpu6050的寄存器地址
 * @param length 需要写入数据长度
 * @param data 写入数据的起始地址
 */
static inline int8_t i2c_write(unsigned char slave_addr, unsigned char reg_addr,
                             unsigned char length, unsigned char const *data)
{
#define I2C_MAX_SEND_LEN 16

    if(length > I2C_MAX_SEND_LEN)
    {
        return -1;
    }
    uint8_t buff[I2C_MAX_SEND_LEN + 1];
    buff[0] = reg_addr;
    memcpy(buff + 1, data, length);

    HAL_I2C_Master_Transmit(&hi2c1, slave_addr << 1, buff, length + 1, HAL_MAX_DELAY);
    return 0;
}

/**
 * @brief 使用i2c读取mpu6050的寄存器
 * 
 * @param slave_addr mpu6050的从机地址
 * @param reg_addr mpu6050的寄存器地址
 * @param length 需要读取的数据长度
 * @param data 用来存储读取到的数据
 */
static inline int8_t i2c_read(unsigned char slave_addr, unsigned char reg_addr,
                            unsigned char length, unsigned char *data)
{
    HAL_I2C_Master_Transmit(&hi2c1, slave_addr << 1, &reg_addr, 1, HAL_MAX_DELAY);
